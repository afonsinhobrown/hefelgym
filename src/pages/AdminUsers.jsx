...
import { User, Shield, Briefcase, Plus, X, Lock, Trash2, Edit, CheckCircle, Ban, Search, Power } from 'lucide-react';
...
const AdminUsers = () => {
    ...
    const getRoleBadge = (role) => {
        ...
        switch (role) {
            case 'gym_admin': return <span style={{ ...badgeStyle, backgroundColor: 'rgba(239, 68, 68, 0.1)', color: '#f87171', borderColor: 'rgba(239, 68, 68, 0.2)' }}><Shield size={12} /> ADMIN</span>;
            case 'manager': return <span style={{ ...badgeStyle, backgroundColor: 'rgba(59, 130, 246, 0.1)', color: '#60a5fa', borderColor: 'rgba(59, 130, 246, 0.2)' }}><Briefcase size={12} /> GERENTE</span>;
            default: return <span style={{ ...badgeStyle, backgroundColor: 'rgba(16, 185, 129, 0.1)', color: '#34d399', borderColor: 'rgba(16, 185, 129, 0.2)' }}><User size={12} /> OPERADOR</span>;
        }
    };
        ...
        <div style={styles.container}>
            <div style={styles.header}>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                    <div style={{ position: 'relative' }}>
                        ...
                        <input type="text" placeholder="Buscar utilizador..."
                            style={{ backgroundColor: '#0f172a', border: '1px solid #334155', borderRadius: '6px', padding: '8px 8px 8px 36px', color: 'white', outline: 'none', width: '250px' }}
                            value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    </div>
                    <button onClick={() => { setFormData({ id: null, name: '', email: '', password: '', role: 'operator', status: 'active' }); setShowModal(true); }}
                        style={{ backgroundColor: '#2563eb', color: 'white', padding: '8px 16px', borderRadius: '6px', border: 'none', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
            ...
            <div style={styles.card}>
                <table style={styles.table}>
                    <thead>
                        <tr>
                            <th style={{ ...styles.th, width: '30%' }}>Nome / Staff</th>
                            <th style={{ ...styles.th, width: '25%' }}>Email (Login)</th>
                    ...
                    <tbody>
                        {loading ? (
                            <tr><td colSpan="5" style={{ ...styles.td, textAlign: 'center', color: '#64748b' }}>A carregar...</td></tr>
                        ) : filteredUsers.length === 0 ? (
                            <tr><td colSpan="5" style={{ ...styles.td, textAlign: 'center', color: '#64748b' }}>Nenhum utilizador encontrado.</td></tr>
                        ) : filteredUsers.map((u) => (
                            <tr key={u.id} style={{ backgroundColor: u.status === 'inactive' ? 'rgba(239, 68, 68, 0.05)' : 'transparent', opacity: u.status === 'inactive' ? 0.7 : 1 }}
                                onMouseEnter={e => e.currentTarget.style.backgroundColor = '#334155'}
                                onMouseLeave={e => e.currentTarget.style.backgroundColor = u.status === 'inactive' ? 'rgba(239, 68, 68, 0.05)' : 'transparent'}>
                                <td style={{ ...styles.td }}>
                                ...
                                <td style={{ ...styles.td, fontFamily: 'monospace', fontSize: '13px' }}>{u.email}</td>
                                <td style={{ ...styles.td, textAlign: 'center' }}>{getRoleBadge(u.role)}</td>
                                <td style={{ ...styles.td, textAlign: 'center' }}>
                                    <span style={{
                                        padding: '4px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: 'bold',
                                        backgroundColor: u.status === 'inactive' ? 'rgba(239, 68, 68, 0.2)' : 'rgba(34, 197, 94, 0.2)',
                                        color: u.status === 'inactive' ? '#fca5a5' : '#86efac',
                                        border: `1px solid ${u.status === 'inactive' ? 'rgba(239, 68, 68, 0.3)' : 'rgba(34, 197, 94, 0.3)'}`
                                    }}>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'center', gap: '8px' }}>
                                        {u.role !== 'gym_admin' && (
                                            <button onClick={() => handleToggleStatus(u)}
                                                style={{ padding: '8px', borderRadius: '6px', border: 'none', backgroundColor: u.status === 'active' ? '#f59e0b' : '#10b981', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                                                title={u.status === 'active' ? "Desativar Acesso" : "Ativar Acesso"}>
                                                {u.status === 'active' ? <Ban size={16} /> : <CheckCircle size={16} />}
                                            </button>
                                        )}
                                        <button onClick={() => openEdit(u)} style={{ padding: '8px', borderRadius: '6px', border: 'none', backgroundColor: '#3b82f6', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Editar">
                                            <Edit size={16} />
                                        </button>
                                        {u.role !== 'gym_admin' && (
                                            <button onClick={() => { if (window.confirm(`Tem a certeza que deseja remover DEFINITIVAMENTE o acesso de "${u.name}"?\nIsso NÃO apaga o funcionário da base de dados, apenas o login.`)) handleDelete(u.id); }}
                                                style={{ padding: '8px', borderRadius: '6px', border: 'none', backgroundColor: '#ef4444', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Eliminar Login">
                                                <Trash2 size={16} />
                                            </button>
                                        )}
                                    </div>
                        ...
                        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            <div style={{ backgroundColor: '#1e293b', padding: '16px', borderRadius: '12px', border: '1px solid #334155', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                <div>
                                    ...
                                    <select style={{ width: '100%', backgroundColor: '#020617', border: '1px solid #334155', borderRadius: '8px', padding: '10px', color: 'white', outline: 'none' }}
                                        value={formData.staffId || ''}
                                        onChange={(e) => {
                                            const selectedId = e.target.value;
                                            const staff = staffList.find(s => String(s.id) === String(selectedId));
                                            if (staff) {
                                                setFormData({ ...formData, staffId: staff.id, name: staff.name, email: staff.email || `${staff.name.toLowerCase().split(' ')[0]}@hefelgym.com` });
                                            } else { setFormData({ ...formData, staffId: '', name: '' }); }
                                        }} disabled={!!formData.id}>
                                        <option value="">Selecione um funcionário...</option>
                                        {staffList.map(s => (<option key={s.id} value={s.id}>{s.name} ({s.type === 'manager' ? 'Gerência' : 'Staff'})</option>))}
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input type="email" style={{ width: '100%', backgroundColor: '#020617', border: '1px solid #334155', borderRadius: '8px', padding: '10px', color: 'white', outline: 'none', fontFamily: 'monospace' }}
                                        required value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} placeholder="usuario@hefelgym.com" />
                                </div>
                                <div>
                                    <label style={{ display: 'block', color: '#94a3b8', fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '4px' }}>
                                        Senha {formData.id && <span style={{ fontSize: '10px', color: '#eab308', fontWeight: 'normal', textTransform: 'none', marginLeft: '6px' }}>(Deixe em branco para manter)</span>}
                                    </label>
                                    <div style={{ position: 'relative' }}>
                                        <input type="text" style={{ width: '100%', backgroundColor: '#020617', border: '1px solid #334155', borderRadius: '8px', padding: '10px 10px 10px 36px', color: 'white', outline: 'none', fontFamily: 'monospace' }}
                                            required={!formData.id} value={formData.password} onChange={e => setFormData({ ...formData, password: e.target.value })} placeholder="••••••••" />
                                        <Lock size={14} style={{ position: 'absolute', left: '12px', top: '14px', color: '#64748b' }} />
                                    </div>
                                </div>
                                <div style={{ display: 'flex', gap: '12px' }}>
                                    <div style={{ flex: 1 }}>
                                        <label style={{ display: 'block', color: '#94a3b8', fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '4px' }}>Função</label>
                                        <select style={{ width: '100%', backgroundColor: '#020617', border: '1px solid #334155', borderRadius: '8px', padding: '10px', color: 'white', outline: 'none' }}
                                    ...
                                    <div style={{ flex: 1 }}>
                                        <label style={{ display: 'block', color: '#94a3b8', fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '4px' }}>Status</label>
                                        <select style={{ width: '100%', backgroundColor: '#020617', border: '1px solid #334155', borderRadius: '8px', padding: '10px', color: 'white', outline: 'none' }}
                            ...
                            <div style={{ display: 'flex', gap: '12px' }}>
                                <button type="button" onClick={() => setShowModal(false)} style={{ flex: 1, padding: '12px', borderRadius: '8px', backgroundColor: '#334155', color: 'white', border: '1px solid #475569', fontWeight: 'bold', cursor: 'pointer' }}>Cancelar</button>
