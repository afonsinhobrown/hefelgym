...
// ==========================================
// TESTE DE ACESSOS SEM FILTRO (RAW)
// ==========================================
db.get("SELECT * FROM access_devices LIMIT 1", [], async (err, device) => {
    if (err || !device) return;

    console.log(`🔍 Buscando últimos 10 eventos EM ${device.ip} (RAW)...`);
    ...
    try {
        ...
        if (response.statusCode === 200) {
            ...
            const events = json.AcsEventSearch?.AcsEventSearchResult?.MatchList || [];

            console.log("\n--- EVENTOS RETORNADOS NA LISTA BRUTA ---");
            if (events.length === 0) console.log("⚠️ Lista vazia!");

            events.forEach(e => {
                const evt = e.AcsEvent;
                console.log(`📅 DATA NA CATRACA: [${evt.time}] | Nome: ${evt.name}`);
            });
            console.log("\nSe as datas acima forem de 2025, o filtro 'isToday' (2026) está bloqueando.");
        } else {
