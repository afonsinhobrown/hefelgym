...
// ==========================================
// TESTE V3 - COM MAJOR/MINOR
// ==========================================
db.get("SELECT * FROM access_devices LIMIT 1", [], async (err, device) => {
    if (err || !device) { console.log("Sem dispositivo"); return; }

    console.log(`🔍 ESCANEANDO (V3 FIXED) EM ${device.ip}...`);

    const jsonPayload = JSON.stringify({
        AcsEventCond: {
            searchID: "TEST_JSON_V3",
            searchResultPosition: 0,
            maxResults: 15,
            major: 5,
            minor: 75, // Tentar acesso normal. Se falhar tento 0,0
    ...
    try {
        ...
        console.log("BODY:", response.data.substring(0, 1000));
